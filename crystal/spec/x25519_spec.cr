...
PRIVATE_KEY     = Bytes[112, 30, 183, 213, 243, 4, 148, 142, 152, 159, 212, 16, 178, 101, 31, 234, 60, 109, 74, 131, 231, 29, 226, 172, 174, 10, 83, 80, 18, 93, 112, 93]
PUBLIC_KEY      = Bytes[190, 1, 168, 92, 39, 211, 30, 82, 66, 57, 88, 184, 67, 86, 207, 225, 188, 130, 47, 245, 223, 88, 66, 99, 184, 66, 62, 163, 168, 32, 0, 72]
PRIVATE_KEY_PEM = <<-PEM
-----BEGIN PRIVATE KEY-----
...
describe OpenSSL::PKey::X25519 do
  ...
  it "writes to pem" do
    priv = X25519.from_bytes PRIVATE_KEY, true
    priv.to_pem.should eq PRIVATE_KEY_PEM

    pub = X25519.from_bytes PUBLIC_KEY, false
    pub.to_pem.should eq PUBLIC_KEY_PEM

    priv.public_key.to_pem.should eq PUBLIC_KEY_PEM
  end
